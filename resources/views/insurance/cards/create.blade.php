<x-app-layout>
    <x-slot name="heading">{{ __('insurance.new_card') }}</x-slot>

    <div class="max-w-2xl">
        <x-page-header :title="__('insurance.new_card')" :back="route('insurance.cards.index')" />

        <form method="POST" action="{{ route('insurance.cards.store') }}" class="space-y-4">
            @csrf
            ...
            <x-card class="p-5 space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select id="insurance_company_id" name="insurance_company_id" required
                            ...
                            @foreach($companies as $co)
                                <option value="{{ $co->id }}" @selected(old('insurance_company_id', request('company')) == $co->id)>{{ $co->name }}</option>
                            @endforeach
                        </select>
                        <x-input-error :messages="$errors->get('insurance_company_id')" class="mt-1" />
                    </div>
                    <div>
                        <x-input-label for="card_number" :value="__('insurance.card_number') . ' *'" />
                        <x-text-input id="card_number" name="card_number" type="text" class="mt-1 block w-full" :value="old('card_number')" required />
                    ...
                    <div>
                        ...
                        <x-text-input id="employer_name" name="employer_name" type="text" class="mt-1 block w-full" :value="old('employer_name')" />
                    </div>
                    <div>
                        ...
                        <x-text-input id="coverage_pct" name="coverage_pct" type="number" step="0.01" min="0" max="100"
                            class="mt-1 block w-full" :value="old('coverage_pct')" placeholder="{{ __('insurance.leave_for_default') }}" />
                        <p class="mt-1 text-xs text-gray-400">{{ __('insurance.coverage_override_hint') }}</p>
                    </div>
                    <div>
                        ...
                        <x-text-input id="copay_amount" name="copay_amount" type="number" step="0.01" min="0"
                            class="mt-1 block w-full" :value="old('copay_amount', 0)" />
                    </div>
                    <div>
                        <x-input-label for="valid_from" :value="__('insurance.valid_from')" />
                        <x-text-input id="valid_from" name="valid_from" type="date" class="mt-1 block w-full" :value="old('valid_from')" />
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <x-text-input id="monthly_limit" name="monthly_limit" type="number" step="0.01" min="0"
                            class="mt-1 block w-full" :value="old('monthly_limit')" placeholder="{{ __('insurance.no_limit') }}" />
                    </div>
                    <div>
                        ...
                        <x-text-input id="coverage_limit_annual" name="coverage_limit_annual" type="number" step="0.01" min="0"
                            class="mt-1 block w-full" :value="old('coverage_limit_annual')" placeholder="{{ __('insurance.no_limit') }}" />
                    </div>
