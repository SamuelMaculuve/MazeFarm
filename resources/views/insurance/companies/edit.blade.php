<x-app-layout>
    ...
    <div class="max-w-2xl">
        <x-page-header :title="__('insurance.edit_company')" :back="route('insurance.companies.show', $company)" />

        <form method="POST" action="{{ route('insurance.companies.update', $company) }}" class="space-y-4">
            @csrf @method('PATCH')
            ...
            <x-card class="p-5 space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <x-text-input id="nuit" name="nuit" type="text" class="mt-1 block w-full" :value="old('nuit', $company->nuit)" />
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select id="billing_cycle" name="billing_cycle" required
                            ...
                            <option value="monthly" @selected(old('billing_cycle', $company->billing_cycle) === 'monthly')>{{ __('insurance.monthly') }}</option>
                            <option value="weekly" @selected(old('billing_cycle', $company->billing_cycle) === 'weekly')>{{ __('insurance.weekly') }}</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <x-text-input id="default_coverage_pct" name="default_coverage_pct" type="number" step="0.01" min="0" max="100"
                            class="mt-1 block w-full" :value="old('default_coverage_pct', $company->default_coverage_pct)" />
                    </div>
                    <div class="md:col-span-2">
                        <x-input-label for="address" :value="__('common.address')" />
                        <textarea id="address" name="address" rows="2"
                            class="mt-1 block w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-sm focus:outline-none focus:border-gray-400">{{ old('address', $company->address) }}</textarea>
                    </div>
                    <div class="md:col-span-2 space-y-2">
                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="requires_preauth" value="1" @checked(old('requires_preauth', $company->requires_preauth)) class="w-4 h-4 rounded border-gray-300">
                            <span class="text-sm font-medium text-gray-900">{{ __('insurance.requires_preauth') }}</span>
                        </label>
                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $company->is_active)) class="w-4 h-4 rounded border-gray-300">
            ...
            <div class="flex items-center justify-end gap-3">
                <a href="{{ route('insurance.companies.show', $company) }}" class="px-4 py-2 text-sm font-medium text-gray-600 hover:text-gray-900">{{ __('common.cancel') }}</a>
